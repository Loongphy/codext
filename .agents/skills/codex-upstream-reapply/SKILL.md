...
### 6.2) Bug-fix 条目：先检查上游是否已修复

CHANGED.md 中属于 bug fix（针对某个上游 regression 的修复，而非长期二开行为）的条目，在重实现之前必须先确认上游状态，避免与上游自己的修复重复或冲突：

1. 打开条目中记录的上游 issue/PR 链接确认其状态，并在 NEW_BRANCH 对应的 TAG 源码里直接检查相关实现是否已经变化（例如上游已恢复、改写或以其他方式修复了同一逻辑）。
2. 若上游已在新 TAG 中修复同一问题：**不要重实现本条目**；同时删除 CHANGED.md 中对应条目和 README.md 里相关的 BUG FIX 小节，保持文档与实际 fork 行为一致。
3. 若上游未修复：按条目要求重新实现，并保留 CHANGED.md / README.md 记录。
4. 在最终回复中报告判定结果：哪些 bug-fix 条目被上游吸收（已删除记录），哪些被重新实现。

判断“是否同一个问题”以根因和用户可见症状为准，不以文件路径或函数名为准；上游可能在不同模块用不同方式修复同一问题。CHANGED.md 中带有 "Reapply policy" 引导块的条目都遵循本节流程；没有显式标注的 bug-fix 类条目同样适用。
