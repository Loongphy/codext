...
- The status header row should be preceded by a 1-line top inset
  (`Insets::tlbr(/*top*/ 1, …)`) to maintain consistent vertical rhythm with
  the active cell and active hook cell renderables that appear above it in the
  same column layout.
- Content is left-indented by the same gutter width used by historical cells
  and the configurable status-line footer, so the header visually aligns with
  transcript content rather than the full terminal width.
