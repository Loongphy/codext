...
- The status header row is preceded by a 1-line top inset (`Insets::tlbr(/*top*/ 1, …)`), matching the top spacing used by the active cell and active hook cell renderables within the same column layout.
...
- All subsequent status header modifications must follow the `status-header` skill (`.agents/skills/status-header/SKILL.md`).
...
- `account/read` now accepts `reloadAuthFromStorage: true` to force a storage reload before returning account state. This path also uses the app-server idle guard, so a client that calls it while a turn is running gets the cached account state without updating the shared `AuthManager` during that active turn.
