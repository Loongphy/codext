...
const AUTH_RELOAD_DEBOUNCE_DELAY: Duration = Duration::from_millis(200);
    ...
    loop {
        ...
        match watch_rx.recv_timeout(AUTH_POLL_INTERVAL) {
            Ok(Ok(event)) => {
                ...
                // Editors often replace auth.json through several filesystem events. Wait for
                // a quiet period so the reload observes the settled file instead of a partial
                // write.
                loop {
                    match watch_rx.recv_timeout(AUTH_RELOAD_DEBOUNCE_DELAY) {
                        Ok(Ok(next_event)) if is_relevant_kind(next_event.kind) => continue,
                        Ok(Ok(_)) | Ok(Err(_)) | Err(mpsc::RecvTimeoutError::Timeout) => break,
                        Err(mpsc::RecvTimeoutError::Disconnected) => {
                            return Some(auth_file_state(auth_path));
                        }
                    }
                }
