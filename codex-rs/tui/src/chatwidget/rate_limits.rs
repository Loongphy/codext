...
impl ChatWidget {
    /// Poll at least every 15s for every ChatGPT account, tightening near exhaustion.
    pub(crate) fn rate_limit_refresh_interval(&self) -> Option<std::time::Duration> {
        ...
        let used_percent = self
            ...
            .unwrap_or_default();
        // Poll at least every 15s so the status header and `/statusline` limit
        // items keep moving while the UI is otherwise idle; near exhaustion the
        // upstream cadence tightens further.
        let seconds = if used_percent >= 99.0 { 5 } else { 15 };
