...
//! inside the bottom section, directly above the composer.
//!
//! To keep the visual rhythm consistent across the TUI:
//!
//! - The enclosing bottom section already contributes the 1-line gap above this
//!   surface, so the status header itself does not add another top inset.
//! - The content is left-indented by [`LIVE_PREFIX_COLS`] columns, matching the
