...
echo "[INFO] Copying README.md and CHANGED.md from ${OLD_BRANCH} (no modifications)..."
copy_file_from_old_branch "${OLD_BRANCH}" "README.md"
copy_file_from_old_branch "${OLD_BRANCH}" "CHANGED.md"
if ! git diff --cached --quiet; then
  if git commit -m "docs: copy README.md and CHANGED.md from ${OLD_BRANCH}"; then
    echo "[OK] Committed README.md and CHANGED.md copy"
  else
    echo "[WARN] Unable to commit copied docs (git user.name/user.email?)."
    echo "[WARN] Commit manually with: git commit -m \"docs: copy README.md and CHANGED.md from ${OLD_BRANCH}\""
  fi
fi
